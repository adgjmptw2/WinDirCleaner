...
- **권장 / 선택 / 위험**으로 나눈 정리 후보 목록을 보여 줍니다. 앱 시작 시에는 **정책·UI 연습용 정적 샘플**이 먼저 표시됩니다.
- **정리 후보 새로고침**을 누르면(데모 모드가 꺼진 경우) 일부 경로에 대해 **읽기 전용으로 폴더 크기만** 계산해 목록을 갱신합니다. 파일을 수정·삭제하지 않습니다.
- **현재 읽기 전용으로 크기를 확인할 수 있는 후보:** 사용자 Temp, `%SystemRoot%\SoftwareDistribution\Download`(Windows 업데이트 다운로드 캐시), `%LOCALAPPDATA%\CrashDumps`, `%SystemRoot%\Minidump`.
- **보호(위험) 항목:** WinSxS, DriverStore, Program Files — **크기를 계산하지 않으며** UI에는 **보호됨**으로 표시되고 체크할 수 없습니다.
- **정리 미리보기** 버튼으로, 체크한 **권장·선택** 후보에 한해 **읽기 전용으로** 대상 파일 수·폴더 수·예상 크기 합계와 **샘플 경로 일부**만 계산해 보여 줍니다. **파일을 삭제하거나 바꾸지 않습니다.** 위험 분류 항목은 미리보기 대상에서도 제외됩니다.
- 데모 모드에서는 실제 폴더를 읽지 않고, **고정된 샘플 요약·샘플 경로**만 표시합니다.
- **삭제 실행·정리 실행 버튼은 없습니다.** 정리 미리보기는 읽기 전용 집계만 합니다. 체크한 항목 수와(표시된 크기가 있는 경우)합계만 하단에 집계합니다. “삭제 가능” 같은 표현은 쓰지 않습니다.
...
- 파일 삭제, **삭제를 수행하는** 삭제 미리보기, 정리 후보 자동 분류·일괄 삭제
- 전체 트리 시각화, 최근 폴더 저장
